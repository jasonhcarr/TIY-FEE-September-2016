(function(){
  ...
  var fetchLeague = {
    ...
    generateTemplate: function(page) {
      var pageTemplate = page === 'home' ? page +'-template' : 'dog-template';
      ...
      if (page === 'abbie') {
        context = this.dogs.abbie;
      } else if (page === 'ragnar') {
        context = this.dogs.ragnar;
      } else {
        context = this.dogs.sterling;
      }

      var html = template(context);
      $('.content-container').fadeOut('fast', function() {
        $(this).html('').prepend(html).fadeIn();
      });
    },
